...
// ── Модель ──────────────────────────────────────────────────────────────────
// Продажи месяца T = сумма по месяцам M от T назад до T−8 и по каналам:
//   встречи(M, канал) × конверсия(канал) × доля лага(канал, T − M)
// Слагаемое с M = T (лаг 0) входит наравне с остальными: часть сделок
// закрывается в месяц первичной встречи — 14.3% горячих и 14.6% холодных.
...
function buildModel() {
  ...
  const closesOf = (T) => {
    const out = {
      lo: 0, mid: 0, hi: 0,
      fromKnown: 0, fromModel: 0,
      fromSameMonth: 0, // вклад встреч самого месяца T (лаг 0)
      byChannel: { hot: 0, cold: 0 },
    };
    // k — лаг в месяцах; k = 0 это сам месяц T, его встречи тоже дают продажи
    ...
    <div>
      ...
      <div style={card}>
        ...
        <div style={{ fontSize: 11, color: "var(--color-text-secondary,#757987)", marginTop: 12, lineHeight: 1.6 }}>
          <b>Как считается прогноз на месяц.</b> Продажи месяца T = сумма по месяцам M от самого
          T назад до T−8 и по обоим каналам: встречи(M, канал) × конверсия канала × доля сделок
          канала, закрывающихся ровно через (T − M) месяцев. Слагаемое с M = T тоже входит:{" "}
          {(WIN_LAG.hot[0] * 100).toFixed(1)}% горячих сделок и{" "}
          {(WIN_LAG.cold[0] * 100).toFixed(1)}% холодных закрываются в месяц первичной встречи.
          Но основную часть месяца наполняют встречи прошлых месяцев — на собственные приходится
          {" "}
          {(
            (rows.reduce((a, r) => a + r.fromSameMonth, 0) /
              rows.reduce((a, r) => a + r.mid, 0)) *
            100
          ).toFixed(0)}
          % прогноза за год, остальное дозревает с прошлых.
          <br />
